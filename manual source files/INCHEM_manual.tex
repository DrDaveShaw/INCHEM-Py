...
\title{Indoor Chemical Model (INCHEM-Py v1.x) \\
User Manual\\
\begin{figure}
...
When using custom inputs it is important that any new species do not share a name with any of the existing species, including those used in the INCHEM\_Additional file inputs.\seealso{\hyperref[custom_input.txt]{custom\_input.txt}}
...
\label{particles}
\texttt{1 or 0}\\
Set to 1 to include gas-to-particle partitioning for limonene, alpha-pinene and beta-pinene within the simulation and to 0 to exclude particle formation. Details of how particles are implemented can be found in Carslaw et al. (2012), which is based on the methodology of Pankow (1994) \cite{Carslaw2012,Pankow1994}.

\index{INCHEM\_additional}\marginlabel{INCHEM\_additional}
\texttt{1 or 0}\\
Set as 1 to use the INCHEM\_additional.py module which includes additional reaction mechanisms developed specifically for indoor air chemistry. \seealso{\hyperref[INCHEM_additional.py]{INCHEM reactions}}

\index{custom}\marginlabel{custom}
\texttt{1 or 0}\\
Set as 1 to use the custom\_input.txt file to include user set reactions that are not included in the MCM. Set as 0 to ignore custom\_input.txt. The formatting of this file is detailed within the file itself and also within the \seealso{\hyperref[custom_input.txt]{custom\_input.txt}} custom\_input.txt section of this document.
...
\texttt{"glass\_C", "low\_emissivity", or "low\_emissivity\_film"}\\
Type of window glass used for the attenuation of outdoor light by wavelength range as a string. The values are given in the photolysis.py module \seealso{\hyperref[photolysis.py]{photolysis.py}} and are based on the paper by Blocquet et al., (2018) \cite{Blocquet2018}.
...
\texttt{1 or 0}\\
Initial gas concentrations are either provided by a text file (when initials\_from\_run = 0 and initial\_conditions\_gas is provided with the name of a text file) or by an output file from a previous run (when initials\_from\_run = 1 and providing an input as detailed below). 
...
\texttt{0 or 1}\\
Set as 0 to not include additional emissions and 1 to include additional emissions. Emissions can be added at specific points in time during the simulation. The times and emission rates are set using timed\_inputs.
...
\texttt{0 or 1}\\
1 to produce a graph of selected species (output\_species) and write it to file as "graph.png" in the output folder, 0 to not produce a graph. The species chosen will also have their concentrations saved in a csv format in the output folder in molecules cm$^{-3}$.
...
A graph of species concentrations with time for the species specified in the output\_species variable of settings.py. This is only produced if output\_graph = 1.
...
\subsection{Additonal INCHEM reactions}\label{INCHEM_additional.py}
...
The master array is a dictionary of species and their ordinary differential equations (ODEs) that are processed by the integrator. The ODEs are constructed from the reactions input from the MCM, the INCHEM additional input file, any custom reactions added via the custom inputs file, and the gas-to-particle reactions. 
...
log_plot = 1
\end{verbatim}
Can be 1 or 0. 1 to set the y axis to a log scale, 0 to set the y axis to a linear scale.
